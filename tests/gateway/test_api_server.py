...
class TestAdapterInit:
    ...
    def test_create_agent_uses_authenticated_session_key_as_runtime_user(self, monkeypatch):
        captured = {}
        ...
        adapter = APIServerAdapter(PlatformConfig(enabled=True))
        monkeypatch.setattr(adapter, "_ensure_session_db", lambda: None)
        session_key = "agent:dug:hearth:dm:opaque-user"
        ...
        assert isinstance(agent, FakeAgent)
        assert captured["gateway_session_key"] == session_key
        assert captured["user_id"] == session_key
